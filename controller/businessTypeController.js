...
module.exports = {
    getBusinessTypeList: async (req, res) => {
        try {
            ...
            let count = await BusinessType.aggregate([{ $match: obj }, { $group: { _id: null, count: { $sum: 1 } } }]);
            BusinessType.geBUsinessTypeWithFilter(obj, sortByField, sortOrder, paged, pageSize, (err, resdata) => {
                if (err) {
    ...
    addBusinessTypeData: async (req, res) => {
        try {
            let data = req.body;
            let store = req.store;
            ...
            BusinessType.addBusinessType(data, async (err, resdata) => {
                if (err) {
                    return res.json(helper.showDatabaseErrorResponse("INTERNAL_DB_ERROR", err));
                } else {
    ...
    getBusinessTypeDetailsById: async (req, res) => {
        try {
            ...
            BusinessType.getBusinessTypeByStoreId(Cuisine_obj, (err, resdata) => {
                if (err) {
                    return res.json(helper.showDatabaseErrorResponse("INTERNAL_DB_ERROR", err));
                } else {
    ...
    updateBusinessTypeData: async (req, res) => {
        try {
            var data = req.body;
            if (!data._id) {
                return res.json(helper.showValidationErrorResponse('ID_IS_REQUIRED'));
            ...
            if (!data.name) {
                return res.json(helper.showValidationErrorResponse('BUSINESSTYPE_NAME_IS_REQUIRED'));
            };
            if (!data.status) {
                return res.json(helper.showValidationErrorResponse('STATUS_IS_REQUIRED'));
            };
            ...
            BusinessType.updateBusinessType(data, (err, resdata) => {
                if (err) {
                    return res.json(helper.showDatabaseErrorResponse("INTERNAL_DB_ERROR", err));
                } else {
    ...
    removeBusinessTypeData: async (req, res) => {
        try {
            var data = req.body;
            let store = req.store;
            data.store = store.storeId;
            ...
            BusinessType.removeBusinessType(data._id, async (err, resdata) => {
                if (err) {
                    return res.json(helper.showDatabaseErrorResponse("INTERNAL_DB_ERROR", err));
                } else {
                    if (plan && plan.type && plan.type === "basic")
    ...
    archiveBusinessType: async (req, res) => {
        try {
            let data = req.body;
            if (!data._id) {
                return res.json(helper.showValidationErrorResponse('ID_IS_REQUIRED'));
    ...
    updateStatus: async (req, res) => {
        try {
            ...
            BusinessType.updateStatusByIds(data, update, (err, resdata) => {
                if (err) {
                    return res.json(helper.showDatabaseErrorResponse("INTERNAL_DB_ERROR", err));
                } else {
