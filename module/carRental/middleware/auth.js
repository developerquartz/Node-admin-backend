...
module.exports = {
    ...
    isStoreType: async (req, res, next) => {
        ...
        const getStoreType = await storeType.findOne({ _id: data.storeTypeId, store: store.storeId }, 'taxSettings commission deliveryAreaDriver deliveryAreaVendor noOfDriversPerRequest taxAmount scheduled store')
            .populate({ path: 'store', select: 'tip timezone googleMapKey' })
