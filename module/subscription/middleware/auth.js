...
module.exports = {
    ...
    isStoreType: async (req, res, next) => {
        ...
        const getStoreType = await storeType.findById(data.storeTypeId, 'deliveryAreaDriver deliveryAreaVendor noOfDriversPerRequest taxAmount scheduled store taxSettings');
    ...
    isValidVendor: async (req, res, next) => {
        let data = req.body;
        let store = req.store;
        if (!data.vendor) return res.json(helper.showValidationErrorResponse("VENDOR_ID_IS_REQUIRED", data));

        const user = await User.findOne({ _id: data.vendor, store: store.storeId, storeType: data.storeTypeId, status: "approved", role: "VENDOR" })
    ...
    authAdminAndStaffVendorAndUser: async (req, res, next) => {
        ...
        try {
            ...
            const user = await User.findOne({
                status: { $nin: ['archived', 'temp'] },
                $or: [{ _id: decoded._id, role: "ADMIN", "tokens.token": token },
                { _id: decoded._id, role: "USER", "tokens.token": token },
                { _id: decoded._id, role: "VENDOR", "tokens.token": token },
                { _id: decoded._id, role: "STAFF", "tokens.token": token }]
            }).populate({
                path: "store", select: "status timezone storeVersion",
                match: { status: { $ne: 'archived' } }, populate: { path: "plan.billingPlan" }
            }).exec();
